module WOSClient
  class Client
    ...
    def retrieveById(query = @search_xml)
      authenticate if @session_cookie.nil?
      @search_client ||= @client.new(wsdl: @search_url, headers: { 'Cookie' => @session_cookie })
      @last_search = @search_client.call(:retrieve_by_id, soap_action: '', message: query)
    end
    ...
    def query_builder(opts = {})
      ...
      xml_s += '  </editions>'
      if !opts[:symbolic_time_span].nil?
    ...
    def query_retrieve_by_id(query_ut)
      "
